...
//		}
    ...
    public static List<ContributeContract> show_contribute_contract() {
        ...
        try {
            ...
            String sql = "select name,userName,contract_state.time as time from contract_state,contract where contract_state.num=contract.num and type=1";
            System.out.println("生成的sql语句是：" + sql);
            ResultSet rs = jdbc.executeQuery(sql);
            while (rs.next()) {
                contract_state_list.add(new ContributeContract(rs.getString("name"),rs.getString("userName"),rs.getString("time")));
            }
            jdbc.close();
        } catch (Exception e) {
            e.printStackTrace();
